...
# Handle commands that require additional input
            ...
            if not app_handler.open_application(app_name):  # Use app_handler instance
                app_handler.open_application_fallback(app_name)  # Use fallback if opening fails

        # Close application command
        elif "close application" in command:
            app_name = command.replace("close application ", "").strip()
            app_handler.close_application(app_name)  # Directly use app_handler to close the application
  #----------------------------------------------------------------------------         
