...
def open_application(app_name):
    ...
    # If the app is not found in applications_paths, try common directories
    if not app_command:
        add_custom_app(app_name)  # This will attempt to add it to applications_paths
        app_command = applications_paths.get(app_name)  # Try getting it again after adding

    # Check if the app is in the predefined dictionary
    if app_command:
        try:
            speak_text(f"Opening {app_name}...")
            subprocess.Popen([app_command])  # Open the application using its path
    ...
    # If app_name is not in applications_paths, check if it's in the system PATH
    ...
    # If neither applications_paths nor PATH yielded results, return False
    return False
...
def open_application_fallback(app_name):
    ...
    """
    speak_text(f"Application is not Avilable in path, Searching application {app_name} in start menu...")
       
    search_windows_search_bar(app_name)
...
def ocr_click_on_text(target_text):
    # Capture the screen
    ...
    if target_text.lower() in screenshot_text.lower():
        ...
        for i in range(len(data['text'])):
            if target_text.lower() == data['text'][i].lower():
                # Calculate the center position of the detected word
...
def click_on_element(element_name):
    try:
        # Connect to the active window (you can specify a specific app if needed)
        app = Application(backend="uia").connect(title_re=".*", found_index=0)
        dlg = app.active()

        # Search for the element by name and click it
...
# Fallback mechanism 
def find_and_click_text(element_name):
    ...
    for i, text in enumerate(data['text']):
        if element_name.lower() in text.lower():
            ...
            # Center of the detected text box
            center_x = x + width // 2
            center_y = y + height // 2
...
# Main Function that exucutes all cliking methods
def click_on(target_name):
    """
    ...
    if ocr_click_on_text(target_name):
        return
    # Fall back on OCR if UI Automation fails
    elif click_on_element(target_name):
        return
    # Last fallback method if both UI Automation and OCR fail
    else:
        print(f"Trying find_and_click_text as a last resort for '{target_name}'...")
        find_and_click_text(target_name)
...
commands = {
    ...
    "click on": lambda element_name: click_on(element_name),
...
#-----------------------------------------------------------------------------   
        ...
        # -- This is default command thst exucutes first if seperate option is not avialbel for single tasks--
...
#-----------------------------------------------------------------------------
        # Handle "click on" command
            ...
            commands["click on"](element_name)  # Call the click function with element name
...
#-------------------------------------------------------------------------------
        # Process the command in the main loop
        elif "open website" in command:
            website_url = command.replace("open website ", "")
